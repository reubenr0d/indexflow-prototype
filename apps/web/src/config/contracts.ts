...
// Try to load local deployment files (generated by forge deploy scripts)
// These are optional and only exist when contracts have been deployed locally
let localDeployment: DeploymentConfig | null = null;
let localSpokeDeployment: DeploymentConfig | null = null;

try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  localDeployment = require("./local-deployment.json") as DeploymentConfig;
} catch {
...
try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  localSpokeDeployment = require("./local-spoke-deployment.json") as DeploymentConfig;
} catch {
  // Local spoke deployment not available
}
