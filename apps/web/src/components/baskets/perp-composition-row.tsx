...
function renderDirectionIcon(direction: ExposureDirection) {
  if (direction === "Long") return <ArrowUpRight className="mr-1 h-3 w-3" />;
  if (direction === "Short") return <ArrowDownRight className="mr-1 h-3 w-3" />;
    ...
    <div className="flex flex-col gap-4 px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-2.5 w-2.5 shrink-0 rounded-full bg-app-accent shadow-[0_0_0_4px_color-mix(in_srgb,var(--accent)_16%,transparent)]" />
          <p className="min-w-0 truncate font-medium text-app-text">{assetName}</p>
        </div>
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          >
            {renderDirectionIcon(net.direction)}
            {net.direction}
          </span>
        ...
        <div className="mt-2 max-w-lg">
          ...
          <p className="mt-1 flex items-center justify-between font-mono text-xs text-app-muted">
            <span className="inline-flex items-center gap-1">
              ...
              Long {formatUsd1e30(longSize1e30)}
            </span>
            <span className="inline-flex items-center gap-1">
              <ArrowDownRight className="h-3.5 w-3.5 text-app-danger" />
              Short {formatUsd1e30(shortSize1e30)}
            </span>
          </p>
        </div>
      </div>
