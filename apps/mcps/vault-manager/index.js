...
server.registerTool(
  "wire_asset",
  {
    ...
    description:
      ...
      "The tool independently fetches the live seed price (Yahoo USD, or Bybit index for allowlisted crypto when Yahoo misses) and REJECTS any seed that differs by more than 20% (error_code SEED_PRICE_DEVIATION). " +
      "Returns {success, transactionHash, next_steps}.",
    inputSchema: {
      symbol: z.string().describe("Yahoo Finance ticker (e.g. 'BHP.AX', 'AAPL', 'GLEN.L')"),
      seedPriceUsd: z.number().positive().describe("Current price in USD from yfinance_quote's priceUsd field (e.g. 45.20). Must be within 20% of live seed USD (Yahoo or Bybit index fallback) or the call is rejected."),
  ...
  async ({ symbol, seedPriceUsd, justification }) => {
    try {
      ...
      let live;
      try {
        live = await fetchOracleSeedPriceUsd(symbol);
      } catch (err) {
        return toolError(
          "SEED_PRICE_UNAVAILABLE",
          `Could not fetch live seed USD price for ${symbol}: ${err.message}`,
          "Yahoo Finance may be temporarily unavailable, or the symbol does not resolve. " +
            "For allowlisted crypto, ensure Bybit index is reachable or pass seedPriceUsd from bybit_perp_quote.indexPriceUsd via yfinance_quote. " +
            "Skip this pick on this run if no seed source is available. " +
      ...
      if (!check.ok) {
        const liveLabel =
          live.source === "bybit-index"
            ? `live Bybit index (${live.bybitSymbol})`
            : `live Yahoo (${live.yahooTicker})`;
        return toolError(
          "SEED_PRICE_DEVIATION",
          `seedPriceUsd ${seedPriceUsd} differs from ${liveLabel} priceUsd ${live.priceUsd} (${live.currency} ${live.price}) by ${(check.devBps / 100).toFixed(2)}% (max ${(SEED_PRICE_MAX_DEVIATION_BPS / 100).toFixed(0)}%).`,
          "You must call yfinance_quote on this exact symbol in the SAME turn and pass its `priceUsd` field as `seedPriceUsd`. " +
            "For crypto when Yahoo fails, yfinance_quote may return priceUsd from Bybit index — use that exact value. " +
