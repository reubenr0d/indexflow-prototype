...
| `latestRun` | run detail | Backward-compatible latest-run object with `runId`, `finishedAt`, and `summary`, plus model/network/turns/tool-call/error/reasoning-summary fields. `summary` is the full markdown final agent message. Used by the Vault Thesis card as the fallback body when `thesis` is null, and by the AI Decisions panel to label the most-recent run with a `Latest` pill. |
| `recentRuns[]` | run detail[] | Deduplicated by `runId` and capped (default 25, override with `AGENT_METADATA_RUN_LIMIT`). Written even when a run produced zero write actions, so the basket page can surface no-action decisions. Includes final summary, model, network, turns, tool-call trace, errors, soft failures, risk-officer verdicts, confirmation batches, and Responses API reasoning summaries. Raw hidden chain-of-thought is not stored. |
| `recentActions[]` | `{ tool, justification, timestamp, txHash, agentName, runId, params? }` | LLM-authored justifications attached to MCP write-tool calls. Deduplicated by `txHash` and capped (default 100, override with `AGENT_METADATA_ACTION_LIMIT`). The web app joins entries to `recentRuns[]` by `runId` and renders icon-driven action cards in the AI Decisions panel, which is collapsed by default, then joins entries by `txHash` to the on-chain Vault History rows. The Vault Thesis card additionally derives a "Top picks" chip rail from the latest run's `open_position` actions (long → success tone, short → danger tone). `params` is a per-tool params summary (asset id, side, size, collateral, amount, etc.) emitted by `summarizeActionParams`; see `apps/web/public/agent-metadata/README.md` for the per-tool shape. |
...
The issues channel is **always-on**: every tick it brainstorms broader ideas (a new MCP, a strategy tweak, an investigation against a specific vault) and files them as GitHub Issues for human triage. When the deterministic signal detector sees hard run-log failures (`latest_run_errors`, `new_error_code`, or `recurring_error_code`), the issue agent prioritises a dedupe-friendly runtime-failure investigation before speculative ideas.

Components (all sibling-of-PR-channel; no shared state beyond the run log + the repo-editor MCP):

| Layer | File | Role |
|---|---|---|
| F | `agents/self-improver-issues.md` | Meta-agent that runs every tick. Calls `propose_issue` (a new MCP tool) to draft 1-3 issue ideas across five categories: `new_mcp_or_skill`, `strategy_idea`, `data_gap`, `refactor`, `investigation`. Runtime hard-error signals must produce a stable `(agent, error_code)` investigation unless an equivalent issue is already open. |
...
The issues channel honours the **same allowlist** as the PR channel for any `read_repo_file` calls; `propose_issue` has no path argument so the allow-list is N/A there. The five-category enum (`new_mcp_or_skill` / `strategy_idea` / `data_gap` / `refactor` / `investigation`) is the only knob shaping what kinds of suggestions the channel can file, and the title/body length caps (120 chars / 8 KB) are enforced in `apps/mcps/repo-editor/issue-manifest.js`. Runtime-failure investigations are grounded by `(agent, error_code, runTimestamp)` evidence and may cite vault addresses when the failed run was vault-bound.
