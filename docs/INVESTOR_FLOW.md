...
1. **Choose a chain** — The protocol runs a **hub-and-spoke** topology: Sepolia is the sole perp chain (hub), while spoke chains (Fuji, and potentially 100+) accept deposits only. The web UI shows a **split view** with each chain's routing weight and available capacity.
   ...
   - **Multi-chain deposit:** The UI shows a split breakdown across eligible chains, proportional to their routing weights. You enter the USDC amount **once** and the **deposit confirm modal** bundles approve + deposit per chain — there is no separate "Approve USDC" click. The expandable "Routing & network details" panel shows live per-chain gas estimates and which chains still need an approval based on a pre-flight allowance read. While the deposit is executing you can **minimize** the dialog (X, Esc, overlay tap, or the explicit Minimize button); execution and the global transaction dock keep running, and tapping the corresponding dock card maximizes the dialog back so you can see the live per-chain progress.
...
- `/baskets/[address]` shows **unrealised, realised, and net P&L** tiles (pure RPC via `PerpReader.getVaultPnL`) and a **share price history chart** (indexer snapshots with RPC multi-block fallback).
...
- `/baskets` list cards show **24h / 7d share price trend pills** with live data from the indexer or RPC fallback.
- `/portfolio` displays per-basket and aggregate **cost basis** (net deposited minus redeemed), **P&L**, and **ROI %** — sourced from indexer history with an RPC log-scanning fallback.
