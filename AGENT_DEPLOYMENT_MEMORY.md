...
Last updated: 2026-05-21 (atlas-quality-mcp + quality-matrix-manager vault registration)
...
| Envio | Envio Cloud org `reubenr0d` | HyperIndex deployment | `indexflow-prototype-2` (auto-deploy from `reubenr0d/indexflow-prototype@main`, root `apps/envio`, dev tier) | production | agent | `read`, `deploy`, `update-config` | Single Hasura GraphQL endpoint serving every chain. Auto-deploys on push to `main`; current commit `81a71bc` synced 100% on Sepolia + Fuji. Endpoint: `https://indexer.dev.hyperindex.xyz/caee388/v1/graphql` (per-commit on dev tier — upgrade to paid tier for a stable production URL). Manage via `npx envio-cloud …` (agent has session in `~/.envio-cloud`). | 2026-05-20 |
...
| Ethereum Sepolia | Chain `11155111` | Smart Contract | `VaultAccounting` (`0xb70ce09018bD05D79AdFB1B78d9F42156804b269`) | sepolia | agent | `read`, `deploy`, `update-config` | Replacement perp accounting module that ships the `getVaultPnL` unit-scaling fix (GMX 1e30 → USDC 6-dec). Deployed via `script/MigrateVaultAccounting.s.sol` (tx `0x882a4ab20c84bd5fe3117102f1259eb2cfb9a7326fab0da402e2c4025b41f259`, block `10890734`). All 14 basket assets remapped, `Minestarters ML Picks` (`0x6723…0c38`) closed its 5 open legs on the old VA, withdrew capital (~996 USDC), then `setVaultAccounting(NEW_VA)`. `BasketFactory.vaultAccounting` also pointed at NEW_VA so new baskets pick it up. Owner = deployer wallet `0x36716C8C5D1ae680C78bD0ECc230896556399713`. Previous VA `0xC7aA05CCAEA98dC26B8739A19b79875b1432cE43` left in place but unreferenced by the basket / factory; safe to ignore. **Follow-up wirer grant (2026-05-21 17:06 UTC+05:30):** The migration script omitted `setWirer(BasketFactory, true)` on the new VA, so every `BasketFactory.createBasket` since the migration silently reverted with `"Not authorized"` (the factory's terminal `IPerp(vaultAccounting).registerVault(newBasket)` call is `onlyOwnerOrWirer`). Restored manually by the operator via `cast send 0xb70ce09018bD05D79AdFB1B78d9F42156804b269 "setWirer(address,bool)" 0x9c80b76abd7d1470ba9e8aaa49d128b0a99dfec4 true --rpc-url $SEPOLIA_RPC_URL` (owner-only on the new VA, signed by the deployer wallet) — tx `<TO_FILL_AFTER_USER_RUNS>`. `script/MigrateVaultAccounting.s.sol` patched to include this step in the optional `FACTORY` rewire block so future re-migrations don't repeat the gap. | 2026-05-21 |
## Update Template
