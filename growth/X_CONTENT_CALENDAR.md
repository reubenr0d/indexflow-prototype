...
Status transitions: `seeded` → `polished` → `scheduled` → `posted`. A row marked `seeded` has a complete first-pass draft on disk; a row marked `polished` is post-ready in your voice; `scheduled` means it's queued in your X composer; `posted` means it's live and the `posted_url` is filled in.
...
- `status` — `seeded` \| `polished` \| `scheduled` \| `posted`.
- `posted_url` — filled in after the user posts; closes the analytics loop.

---

## Week 0 — Sun May 24, 2026 (prep day)
