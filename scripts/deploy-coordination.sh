...
sh "$REPO_ROOT/scripts/run-keeper-once.sh" || echo "Warning: keeper-once failed — run: npm run keeper:once"
...
echo ""
echo "Next steps:"
echo "  1. One keeper epoch already ran above; for production run the keeper continuously: cd services/keeper && npm start"
echo "  2. Fund the keeper wallet on both chains (gas)"
echo "  3. Update the frontend / subgraph if needed"
