...
contract Vault is ReentrancyGuard, IVault {
    ...
    function _validateTokens(address _collateralToken, address _indexToken, bool _isLong) private view {
        if (_isLong) {
            ...
            _validate(whitelistedTokens[_indexToken], 42);
            _validate(!stableTokens[_indexToken], 44);
            // Allow stable-collateral longs (synthetic longs with USDC collateral).
            // Original GMX required _collateralToken == _indexToken for longs.
            // When _collateralToken != _indexToken, collateral must be a stablecoin.
            if (_collateralToken != _indexToken) {
                _validate(stableTokens[_collateralToken], 42);
            }
            return;
        }
