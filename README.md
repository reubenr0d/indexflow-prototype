...
- **Per-vault AI Operator panel** — [`apps/web/public/agent-metadata/<vault>.json`](apps/web/public/agent-metadata/) feeds the "AI Activity" card on each `/baskets/<vault>` page (thesis, plus collapsed-by-default run details including no-action runs, reasoning summaries, tool-call traces, decisions, and tx justifications).
...
- [`agents/rwa-treasurer.md`](agents/rwa-treasurer.md) — RWA reserve treasurer; sizes USDY/mUSD/mETH allocation against `rwaTargetBps`.
- [`agents/meth-carry-manager.md`](agents/meth-carry-manager.md) — delta-neutral mETH carry vault (long mETH spot + synthetic ETH short).
- [`agents/rwa-yield-router.md`](agents/rwa-yield-router.md) — rotates the configured reserve token when a competing primitive offers ≥75 bps annualised net advantage.
- [`agents/funding-rate-harvester.md`](agents/funding-rate-harvester.md) — cross-venue funding-rate harvester; uses Bybit as a read-only sentiment input.
- [`agents/smart-money-mirror-manager.md`](agents/smart-money-mirror-manager.md) — Mantle ecosystem smart-money mirror; reads Nansen (or an Envio-only degraded fallback).

[`agents/vault-manager.md`](agents/vault-manager.md) is retired from the CI matrix and remains available for local runs only (`node scripts/agent-runner.mjs vault-manager`).
