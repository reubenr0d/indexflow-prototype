...
# 4. Generate the Paperclip-native mirror (required — plugin v0.9.x classifier
#    needs agents/<slug>/AGENTS.md per-slug subfolders; our canonical layout is
#    flat agents/<slug>.md. The mirror at paperclip/companies/indexflow/ bridges
#    the gap. See paperclip/README.md.)
npm run sync:paperclip
#    Re-run this any time COMPANY.md or a mirrored agent prompt changes.

# 5. In the Paperclip UI:
...
#          (BARE absolute path to the MIRROR SUBDIR — NOT the repo root, NOT file:///...
#           Repo root → plugin discovers canonical COMPANY.md with "No structured
#             contents detected" because flat agents/*.md files don't match classifier.
#           file:/// → routed through git-clone path, fails with "Company not found."
#           Mirror subdir → plugin reads paperclip/companies/indexflow/COMPANY.md +
#             agents/<slug>/AGENTS.md correctly.)
#        OR https://github.com/reubenr0d/indexflow-prototype
#          (GitHub URL → shallow git clone; plugin still finds the tracked mirror at
#           paperclip/companies/indexflow/; commit + push the mirror first.)
#    - Discover → Import as new company (creates the IndexFlow company)
#    - Enable daily auto-sync (overwrite mode is the plugin default)

# 6. Wire each imported agent's process adapter (REQUIRED — the plugin doesn't do this).
#    The import lands agents with adapterType=process but adapterConfig only has
#    the managed-instructions paths — no command, args, cwd, or env. Without this
#    step the first heartbeat fails with "Process adapter missing command".
npm run paperclip:configure-adapters
#    Idempotent. See docs/PAPERCLIP_RUNBOOK.md §Phase 4c.
#    Active mirrored employees today: issue-implementer (callback-only),
#    self-improver-issues (paused — CI cron canonical).
#    Prompt-only risk officers (risk-officer-self-improvement,
#    risk-officer-self-improvement-issues) are documented in this COMPANY.md but
#    NOT mirrored — they're invoked inline by sibling scripts and have no heartbeat.
#    Trading agents + Minestarters family are explicitly out-of-scope and should NOT appear.

# 7. First smoke test: UI → Employees → self-improver-issues → Run now.
#    Watch agents/memory/self-improver-issues/paperclip-heartbeat.json get written.

# 8. Promote brainstorm agents one at a time as needed:
...
#      d. Add the agent's { slug, sourcePath } entry to ACTIVE_RUNNABLE_EMPLOYEES
#         in scripts/sync-paperclip-mirror.mjs AND { slug, requiredEnv, optionalEnv }
#         to ACTIVE_EMPLOYEES in scripts/paperclip-configure-adapters.mjs
#      e. Run `npm run sync:paperclip` to regenerate the mirror
#      f. Commit canonical files + script changes + regenerated mirror in one PR
#      g. Push to main; Paperclip picks it up on next sync (or click Sync now)
#      h. Run `npm run paperclip:configure-adapters` to wire the new agent's adapter
```
