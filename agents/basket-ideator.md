...
- [ ] `agents/skills/vault-themes.md` authored (oracle-coverage matrix, registry-lookup rules, curator-persona taxonomy, Season-narrative alignment heuristics).
- [ ] `apps/mcps/envio-graphql/` built (shared with `broadcast-bot`).
- [ ] Optional: `web-search-mcp` for external market signal — default in this prompt is founder context injection (founder pastes a 2-line "what's trending in mining/AI/RWAs this week" into the run context).
- [ ] Paperclip secret bag: `LLM_API_KEY`, `GH_TOKEN` (issue opener).

This file is declarative until those blockers clear.
